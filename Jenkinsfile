pipeline {
  ...
  stages {
    ...
    stage("DEPLOY & ACTIVATE") {
      steps {
        sh """aws ecs run-task --cluster test --count 1 --launch-type FARGATE --task-definition frontend --network-configuration "awsvpcConfiguration={subnets=[subnet-0c8d052e4834bb21c
